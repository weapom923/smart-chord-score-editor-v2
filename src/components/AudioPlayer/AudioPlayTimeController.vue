  ...
  >
    ...
    >
      <waveform-canvas
        ...
        v-bind:view-sample-begin="$data.$_viewSampleBegin"
        v-bind:view-sample-end="$data.$_viewSampleEnd"
...
type DragMode = 'set_loop_definition' | 'seek' | 'modify_loop_begin' | 'modify_loop_end';
...
const AudioPlaytimeController = defineComponent({
  ...
  data(vm): {
    ...
    $_viewSampleBegin: number,
    $_viewSampleEnd: number,
    $_dragMode?: DragMode,
    ...
    return {
      $_sampleValueScale: 1,
      $_viewSampleBegin: 0,
      $_viewSampleEnd: vm.waveformDecimator.numSamples,
      $_dragMode: undefined,
  ...
  computed: {
    ...
    $_playTimeIndicatorStyle(): CSSProperties | undefined {
      const playTimeOffsetPxFromViewBegin = (this.playTimeSec - this.$_viewTimeBeginSec) * this.$_pxPerSec;
      if (
    ...
    $_loopTimeRangeSec(): { begin: number, end: number } | undefined {
      const getTimeRange = (timeASec: number, timeBSec: number): { begin: number, end: number } => {
        const [ begin, end ] = [ this.$_truncateViewTime(timeASec), this.$_truncateViewTime(timeBSec) ].sort();
        return { begin, end };
      }
      ...
      switch (this.$data.$_dragMode) {
        case 'set_loop_definition':
          if (
            ...
            return getTimeRange(this.$data.$_timeSecOnMousedown, this.$data.$_timeSecOnMousemove);
          }
          break;
        case 'modify_loop_begin':
          if (
            ...
            const modifiedBeginTimeSec = this.loopDefinition.beginTimeSec + this.$data.$_timeSecOnMousemove - this.$data.$_timeSecOnMousedown;
            return getTimeRange(modifiedBeginTimeSec, this.loopDefinition.endTimeSec);
          }
          break;
        case 'modify_loop_end':
          if (
            ...
            const modifiedEndTimeSec = this.loopDefinition.endTimeSec + this.$data.$_timeSecOnMousemove - this.$data.$_timeSecOnMousedown;
            return getTimeRange(this.loopDefinition.beginTimeSec , modifiedEndTimeSec);
          }
          break;
      }
      if (this.loopDefinition) {
        return { begin: this.loopDefinition.beginTimeSec, end: this.loopDefinition.endTimeSec };
      }
      return undefined;
    },
    $_loopTimeRangeIndicatorStyle(): CSSProperties | undefined {
      if (!this.$_loopTimeRangeSec) return undefined;
      if (this.$_loopTimeRangeSec.begin >= this.$_viewTimeEndSec) return undefined;
      if (this.$_loopTimeRangeSec.end < this.$_viewTimeBeginSec) return undefined;
  ...
  methods: {
      ...
      switch (this.$data.$_dragMode) {
        ...
        case 'modify_loop_end':
          if (this.$_loopTimeRangeSec) {
            this.$emit('update:loopDefinition', new AudioPlaybackLoopDefinition(
              this.$_loopTimeRangeSec.begin,
              this.$_loopTimeRangeSec.end,
            ));
          } else {
            this.$emit('update:loopDefinition', undefined);
          }
      ...
      const onWheel = (wheelEvent: WheelEvent): boolean => {
        ...
        if (wheelEvent.ctrlKey) {
          if (wheelEvent.shiftKey) {
            ...
            this.$_changeTimeScale(eventTimeSec, delta);
            return true;
          }
        } else {
    ...
    $_updateViewTimeBeginAndEnd(newViewTimeBeginSec: number, newViewTimeEndSec: number) {
      ...
      if (this.$_viewTimeEndSec <= newViewTimeEndSec) {
        ...
        newViewTimeBeginSec = this.$_truncateViewTime(newViewTimeEndSec - newViewDurationSec);
      } else {
        newViewTimeBeginSec = this.$_truncateViewTime(newViewTimeBeginSec);
        newViewTimeEndSec = this.$_truncateViewTime(newViewTimeBeginSec + newViewDurationSec);
      }
      if (
        (newViewTimeBeginSec === this.$_viewTimeBeginSec) &&
        (newViewTimeEndSec === this.$_viewTimeEndSec)
      ) {
        return false;
      }
    ...
    $_getEventTimeSec(mouseEvent: MouseEvent): number | undefined {
      ...
      return this.$_viewTimeBeginSec + eventTimeOffsetXFromViewBegin * this.$_secPerPx;
    },
    ...
    $_changeTimeScale(anchorTimeSec: number, delta: number) {
      const newDurationSec = this.$_truncateViewDuration(this.$_viewDurationSec * Math.pow(1.003, delta));
